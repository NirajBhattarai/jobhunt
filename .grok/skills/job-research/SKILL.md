...
Do not research everything yourself. Spawn specialist `explore` subagents whose prompts tell them to read a file under `agents/` and return only that agent's Output Format.
...
Write run artifacts under `research/runs/{date}/` when you can. Never invent jobs, companies, dates, salaries, or apply URLs. `UNKNOWN` beats a guess. Do not mark a GitHub README as `VERIFIED`.
